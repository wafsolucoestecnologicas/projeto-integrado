...
export class EditBrokerComponent implements OnInit, OnDestroy {
    ...
    private loadBrokerData(): void {
        const subscription: Subscription = this._activatedRoute.data.subscribe((data: Data) => {
            if (data && data['broker']) {
                ...
                this.formGroup.get('broker')?.patchValue(this.broker);
    ...
    private loadAdressesData(): void {
        const subscription: Subscription = this._activatedRoute.data.subscribe((data: Data) => {
            if (data && data['adresses'] && data['adresses'].length > 0) {
                this.address = data['adresses'].filter(
                    (address: Address) => this.broker.id === address.broker?.id && address.isBroker
                )[0];
    ...
    private parseBroker(form: any): UpdateBroker {
        return {
            name: form.broker.name,
            surname: form.broker.surname,
            email: form.broker.email,
            birthDate: form.broker.birthDate,
            RG: form.broker.RG,
            CPF: form.broker.CPF,
            landline: form.broker.landline,
            cellPhone: form.broker.cellPhone,
            profession: form.broker.profession
        };
    }
    ...
    private updateBroker(): void {
        if (this.broker.id) {
            const subscription: Subscription = this._brokerService
                .update(this.parseBroker(this.formGroup.value), this.broker.id)
