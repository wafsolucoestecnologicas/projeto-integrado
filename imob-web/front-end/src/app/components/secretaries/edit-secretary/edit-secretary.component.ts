...
export class EditSecretaryComponent implements OnInit, OnDestroy {
        ...
        const subscription: Subscription = this._activatedRoute
			...
			.subscribe((data: Data) => {
				if (data && data['secretary']) {
					...
					this.formGroup.get('secretary')?.patchValue(this.secretary);
    ...
    private createFormGroup(): void {
        this.formGroup = this._formBuilder.group({
            secretary: this._formBuilder.group({
    ...
    private parseSecretary(form: any): UpdateSecretary {
        return {
            name: form.secretary.name,
            surname: form.secretary.surname,
            email: form.secretary.email,
            birthDate: form.secretary.birthDate,
            RG: form.secretary.RG,
            CPF: form.secretary.CPF,
            landline: form.secretary.landline,
            cellPhone: form.secretary.cellPhone,
            profession: form.secretary.profession
        };
    }
    ...
    private updateSecretary(): void {
        if (this.secretary.id) {
            const subscription: Subscription = this._secretaryService
                .update(this.parseSecretary(this.formGroup.value), this.secretary.id)
